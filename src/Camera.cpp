...
Camera::Camera(glm::vec3 _pos){
   ...
   m_projectionMatrix = glm::perspective(m_fov, m_aspect, 0.1f, 100.0f);
   lookAt(m_position, glm::vec3(0.0, 1.0, 0.0), m_up);
}
...
void Camera::setShape(float aspect){
    setProjectionMatrix(m_fov, aspect, 0.1f, 100.0f);
}

void Camera::setShape(float _w, float _h){
    setProjectionMatrix(m_fov, _w/_h, 0.1f, 100.0f);
}
